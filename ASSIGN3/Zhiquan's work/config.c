        ...
        if(type == 'G'){
            ...
            for(int j=new_begin; j<MAXSTRING; j++){
                ...
                if((ch[j]=='\t'||ch[j]==' ') && space_num==1 && (ch[j+1]!='\t'&&ch[j+1]!=' ')){
                    ...
                    f->probability = malloc(f->num*sizeof(float));
...
void check_prob(int len, double* pro, int row){
    ...
    for(int i=1;i<len+1;i++){
        ...
        if(pro[i-1]>1){
            fprintf(stderr,"In the [%d] component line, the %dth pro is larger than 1, please check input!", row, i);
            exit(1);
        }
    ...
    if(sum>1.0){
        fprintf(stderr,"In the [%d] component line, the sum of folk's probability is over 1, please check input!", row);
        exit(1);
    }
    
    //Check whether the sum if probability is less than 1.0
    else if(sum<1.0){
        fprintf(stderr,"In the [%d] component line, the sum of the probability is less than 1, please check input!",row);
        exit(1);
    }
}
...
void check_dest_fork(int totalnum, int len, int* des, int row){
    for(int i=1;i<len+1;i++){
        if(des[i-1]>=totalnum){
            fprintf(stderr,"In the [%d] component line, the destination %d doesn't exsit.", row, des[i-1]);
            exit(1);
        }
    }
}
...
void check_dest_queue_gen(int totalnum, int des, int row){
    if(des>=totalnum){
        fprintf(stderr,"In the [%d] component line, the destination %d doesn't exsit.", row, des);
        exit(1);
    }
}
...
void if_numofdest_equal_numofpro(int numofPro, int numofDest, int row){
    if(numofPro!=numofDest){
        fprintf(stderr,"In the [%d] component line, the number of probilities or destinitions are different or less than needed!", row);
        exit(1);
    }
}
