        ...
        if(type == 'G'){
            ...
            for(int j=new_begin; j<MAXSTRING; j++){
                ...
                if((ch[j]=='\t'||ch[j]==' ') && space_num==2 && (ch[j+1]!='\t'&&ch[j+1]!=' ')){
                    ...
                    if(atoi(ch+j+1)!=atof(ch+j+1)){
                        fprintf(stderr, "dest is decimal rather than int in component line [%d]!\n",i);exit(1);
                    }
                    space_num++;
                    continue;
                }
            ...
            if(strlen(ch)>=5){
                printf("!!!\n%d\n!!",strlen(ch));
                fprintf(stderr, "X can't have des or have space followed!\n"); exit(1);
            }
            Exit_num++;
...
void check_dest_queue_gen(int totalnum, int des, int row){
    if(des == row){
        fprintf(stderr,"In the [%d] component line, the destination is itself!\n", row, des);
        exit(1);
    }
    if(des>=totalnum){
        fprintf(stderr,"In the [%d] component line, the destination %d doesn't exsit.\n", row, des);
...
void check_exit_entrance(int totalnum){
    int exit_entry = 0;
        for(int i=0; i<totalnum; i++){
            if(Component_read[i].ComponentType == GENERATOR){
                struct Generator* G_pointer= (struct Generator*)Component_read[i].Comp;
                if(Component_read[G_pointer->DestComp].ComponentType == EXIT) exit_entry++;
            }
            
            if(Component_read[i].ComponentType == FORK){
                struct Fork* F_pointer= (struct Fork*)Component_read[i].Comp;
                
                for(int i=0; i<F_pointer->num; i++){
                    if(Component_read[F_pointer->DestComp[i]].ComponentType == EXIT){
                        exit_entry++;
                    }
                };
            }
            ...
            if(Component_read[i].ComponentType == QUEUE_STATION){
                struct Queue_Station* Q_pointer= (struct Queue_Station*)Component_read[i].Comp;
                if(Component_read[Q_pointer->DestComp].ComponentType == EXIT) exit_entry++;
            }
        }
    if(exit_entry == 0) {
        fprintf(stderr,"Exit has no entry!!!");
    exit(1);
    }
}
