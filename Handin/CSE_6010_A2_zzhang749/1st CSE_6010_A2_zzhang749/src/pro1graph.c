...
int** graph_generate(double prob, int* numberofVertex, int* length){
	//Check the parameter
	if(prob<0 || numberofVertex == NULL || length == NULL) return NULL;
	int len = *length;
	int num = 0;
...
graph* create_graph(int* numberofVertex){
	if(numberofVertex == NULL) return NULL;
	graph* gra = malloc(sizeof(graph));
	if(gra==NULL) return NULL;
	gra->vertex_num = *numberofVertex;
	//gra->vertex = realloc(gra->vertex,(number+1)*sizeof(vertexNode));

	//My vertex starts at 1!So the length of them is number+1!
	gra->vertex = malloc((gra->vertex_num+1)*sizeof(vertexNode));
	if(gra->vertex==NULL) return NULL;
...
graph* check_neighbors(int** grid, graph* gra, int* length){
	//Check the parameter
	if(grid == NULL || gra == NULL || length == NULL) return NULL;
	int len = *length;
	for(int i=1;i<len+1;i++){
		for(int j=1;j<len+1;j++){
			if(grid[i][j]!=0){
...
void show_list(graph* result){
	...
	for(int i=1;i<result->vertex_num+1;i++){
		printf("\n%d",i);
...
void output(graph*result, FILE* fp){
	...
	fprintf(fp, "%d", result->vertex_num);
	for(int i=1;i<result->vertex_num+1;i++){
		fprintf(fp, "\n");
		fprintf(fp, "%d", i);
		if(result->vertex[i].firstedge==NULL) continue;
